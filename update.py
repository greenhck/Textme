...
import pytz
# 1. NEW IMPORT: google.generativeai के बजाय google.genai का उपयोग करें
...
# 2. NEW CLIENT: genai.configure() के बजाय genai.Client() का उपयोग करें
try:
...
def update_aura_scores():
    ...
    try:
        ...
        # NEW: model variable को client का उपयोग करके जनरेट करें
        prompt = get_bulk_aura_change_prompt(celebrity_names)
        
        # --- ROBUST API CALL BLOCK START ---
        
        try:
            # 3. NEW CALL SYNTAX: client.models.generate_content का उपयोग करें
            response = client.models.generate_content(
                model=MODEL_NAME, # मॉडल को आर्गुमेंट के रूप में पास करें
                contents=prompt,
        ...
        # 1. Google-specific exceptions को पकड़ें
        ...
        # 4. Check for empty or blocked response (No change)
        ...
        # 5. Loop through celebrities and update their data (No change)
        ...
        # 6. Update the timestamp to IST (No change)
        ist = pytz.timezone('Asia/Kolkata')
        data['last_updated'] = datetime.now(ist).strftime('%d-%m-%Y %H:%M:%S IST')

        # 7. Write back the updated data (No change)
